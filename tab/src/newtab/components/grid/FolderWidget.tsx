...
function MiniIcon({
  ...
  shortcut: { url: string; title?: string; favicon?: string };
}) {
  const favicon = getFaviconUrl({ url: shortcut.url, favicon: shortcut.favicon });
  ...
  const handleImgError = useCallback(() => {
    ...
    if (!triedIconRef.current) {
      ...
      try {
        ...
        logger.error('[FolderWidget] Failed to get Google favicon:', shortcut.url, error);
      }
    }
...
export const BookmarkFolderWidget = memo(function BookmarkFolderWidget({
  ...
  const { setCurrentFolder, items } = useNewtabStore();
  
  if (item.type !== 'folder') {
    return null;
  }

  const folderData = item.data as { title: string; icon?: string };
  const title = folderData.title || t('default_folder_name');

  const folderRef = useRef<HTMLDivElement>(null);

  const thumbShortcuts = useMemo(() => {
    const children = items
      .filter((i) => (i.parentId ?? null) === item.id && i.type === 'shortcut')
      .sort((a, b) => a.position - b.position)
      .slice(0, 9);
    return children.map((c) => c.data as { url: string; title?: string; favicon?: string });
  }, [items, item.id]);
