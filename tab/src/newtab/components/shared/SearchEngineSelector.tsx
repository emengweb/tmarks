...
function EngineIcon({ icon, name, size = 'md' }: { icon: string; name: string; size?: 'sm' | 'md' }) {
  ...
  if (hasError) {
    return (
      <span className={`fallback-icon ${sizeClass} flex items-center justify-center`}>
        <svg className={`${sizeClass} text-white/60`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
